    ...
    public class DoctorLogic : IDoctorLogic
    {
            ...
            public Modelo.Models.Doctor GetDoctor(int id)
            {
                ...
                try
                {
                    using (Context db = new Context())
                    {
                        result = db.Doctores.Include("Usuario").SingleOrDefault(d => d.DoctorID == id);
                    }
                }
                catch (Exception)
                {
            ...
            public void Edit(Modelo.Models.Doctor doctor)
            {
                try
                {
                    //using (Context db = new Context())
                    //{
                    //    IEnumerable<EspecialidadDoctor> ListEspDoc = db.EspecialidadesDoctores.ToList<EspecialidadDoctor>();
                    //    foreach (EspecialidadDoctor espDoc in ListEspDoc)
                    //    {
                    //        if (espDoc.doctorID == doctor.DoctorID)
                    //        {
                    //            db.Entry(espDoc).State = System.Data.Entity.EntityState.Deleted;
                    //            db.EspecialidadesDoctores.Remove(espDoc);
                    //            db.SaveChanges();
                    //        }

                    //    }
                    //    foreach (Especialidad esp in doctor.ListEspecialidades)
                    //    {
                    //        EspecialidadDoctor espDocToAdd = new EspecialidadDoctor();
                    //        espDocToAdd.doctor = doctor; espDocToAdd.doctorID = doctor.DoctorID;
                    //        espDocToAdd.especialidad = esp; espDocToAdd.EspecialidadID = esp.EspecilidadID;
                    //        db.EspecialidadesDoctores.Add(espDocToAdd);
                    //        db.SaveChanges();
                    //    }
